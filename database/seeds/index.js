...
// Exécuter depuis le dossier backend : npm run seed
require('dotenv').config({ path: '../../backend/.env' });

const bcrypt   = require('bcryptjs');
const path     = require('path');
const { sequelize, User, Enfant, Inscription, Actualite, Menu, CmsPage, Consentement } = require('../../backend/src/models');
...
const ACTUALITES = [
  ...
  },
  {
    ...
    extrait: 'La crèche sera fermée le lundi 2 juin pour une journée pédagogique de l\'équipe. Nous excusons pour la gêne occasionnée.',
  ...
  },
  {
    ...
    extrait: 'Notre cuisinière a concocté un tout nouveau menu de saison pour le mois de juin avec des légumes frais du marché local.',
...
// Menu de la semaine courante
const lundi = new Date();
lundi.setDate(lundi.getDate() - lundi.getDay() + 1); // Lundi de la semaine
...
const CMS_PAGES = [
  {
    ...
    contenu: '<p>La crèche Les Coccinelles est une structure d\'accueil associative implantée à Puilboreau (17). Nous accueillons les enfants de 0 à 3 ans dans un cadre bienveillant et sécurisant.</p>',
    meta_description: 'Découvrez la crèche associative Les Coccinelles à Puilboreau, Charente-Maritime.',
    publie: true
  },
  {
    slug: 'equipe',
    titre: 'Notre équipe',
    contenu: '<p>Notre équipe de professionnels diplômés (EJE, auxiliaires de puériculture, CAP Petite Enfance) accueille vos enfants avec bienveillance et professionnalisme.</p>',
    meta_description: 'Rencontrez l\'équipe de la crèche Les Coccinelles.',
    publie: true
  },
  {
    slug: 'horaires',
    titre: 'Horaires d\'accueil',
    contenu: '<p>La crèche est ouverte du lundi au vendredi de 8h30 à 18h00. Elle est fermée les week-ends et jours fériés.</p>',
    meta_description: 'Horaires d\'ouverture de la crèche Les Coccinelles.',
    publie: true
  },
  {
    slug: 'tarifs',
    titre: 'Tarifs et financement',
    contenu: '<p>Le tarif est calculé par la CAF selon le quotient familial. La participation familiale varie de 0,06€ à 0,60€ par heure selon les revenus. Des aides financières sont disponibles via la CAF et les collectivités locales.</p>',
    meta_description: 'Tarifs et financement de la crèche Les Coccinelles.',
    publie: true
  },
  {
    slug: 'projet-pedagogique',
    titre: 'Projet pédagogique',
    contenu: '<p>Notre projet pédagogique est centré sur trois axes : l\'éveil sensoriel, le lien avec la nature et le respect du rythme de chaque enfant. Nous proposons des activités créatives, des sorties nature et des ateliers musicaux.</p>',
    meta_description: 'Le projet pédagogique de la crèche Les Coccinelles.',
    publie: true
  },
  {
    slug: 'conditions-inscription',
    titre: 'Conditions d\'inscription',
    contenu: '<p>Pour inscrire votre enfant, vous devez constituer un dossier comprenant : carnet de santé, certificat médical, jugement de garde si nécessaire, et justificatifs de revenus pour le calcul du tarif CAF. Contactez-nous pour vérifier les disponibilités.</p>',
    meta_description: 'Conditions d\'inscription à la crèche Les Coccinelles.',
    publie: true
  },
  {
    slug: 'mentions-legales',
    titre: 'Mentions légales & RGPD',
    contenu: '<h2>Éditeur du site</h2><p>Association Les Coccinelles — Puilboreau (17138). Contact : contact@lescoccinelles.fr</p><h2>Données personnelles (RGPD)</h2><p>Les données collectées sont utilisées uniquement dans le cadre de la gestion de la crèche. Conformément au RGPD, vous disposez d\'un droit d\'accès, de rectification et de suppression de vos données.</p>',
    meta_description: 'Mentions légales et politique de confidentialité RGPD.',
    publie: true
  }
];
...
async function seeder() {
  console.log('🌱 Démarrage du seed Les Coccinelles...\n');

  try {
    await sequelize.authenticate();
    console.log('✅ Connexion BDD OK\n');

    // Synchroniser les tables sans détruire les données
    ...
    for (const compte of COMPTES) {
      ...
      console.log(`   ✅ ${compte.role} créé : ${compte.email} / ${compte.password}`);
    }
    ...
    for (const e of enfantsData) {
      ...
      console.log(`   ✅ ${e.prenom} ${e.nom} (parent: ${e.user_id})`);
    }
    ...
    for (const enfant of enfantsCreés) {
      ...
      if (!existant) {
        await Consentement.create({
          enfant_id: enfant.id,
          user_id: enfant.user_id,
          consenti: true,
          date_consentement: new Date(),
          ip_adresse: '127.0.0.1'
        });
        console.log(`   ✅ Consentement accordé pour ${enfant.prenom}`);
      }
    }

    // ── 4. INSCRIPTION TEST ──────────────────────────────────────
    console.log('\n📝 Dossier d\'inscription test...');
    const premierEnfant = enfantsCreés[0];
    if (premierEnfant) {
      const existant = await Inscription.findOne({ where: { enfant_id: premierEnfant.id } });
      if (!existant) {
        await Inscription.create({
          enfant_id: premierEnfant.id,
          user_id: parent1.id,
          statut: 'accepte',
          date_debut_souhaitee: '2025-09-01',
          jours_souhaites: 'lundi,mardi,mercredi,jeudi,vendredi',
          temps_accueil: 'temps_plein',
          commentaire_parent: 'Nous souhaitons un accueil à temps plein pour Lucas.',
          commentaire_admin: 'Dossier complet. Famille prioritaire.',
          traite_par: adminUser.id,
          date_traitement: new Date()
        });
        console.log('   ✅ Inscription Lucas Martin créée (statut: accepte)');
      }
    }
    ...
    if (!menuExistant) {
      await Menu.create({ ...MENU, publie: true, redige_par: adminUser.id });
      console.log(`   ✅ Menu semaine du ${MENU.semaine_debut} créé`);
    } else {
